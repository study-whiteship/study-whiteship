...
최근 들어 함수적 프로그래밍이 다시 부각되고 있는데, 이는 병렬 처리ㄱ와 이벤트 지향 프로그래밍에 적합하기 때문이다. 그래서 객체 제향 프로그래밍과 함수적 프로그래밍을 적절히 혼합함으로써 더욱 효율적인 프로그래밍이 될 수 있도록 언어도 계속 변하고 있다.
...
| Operator  | 매개값 O, 리턴값 X         | 주로 매개값을 연산하고 결과를 리턴       |
