...
###  접근제어자
  ...
  - public : 가능!
  - (None),default : 디폴트 : 같은 패키지 내부에서 접근 가능
  - protected : 패키지 외부의 서브 클래스
<br>

### 클래스의 영역에서
  - 인스턴스 필드 : 각 인스턴스의 속성을 나타낸다
  - 클래스 필드 : 클래스 레벨에서의 속성
  - 생성자 : 새로운 객체를 생성하는데, 여러가지 시그니처로 만들수 있고, 새로운 객체를 생성(정확히는 인스턴스)하는 용
  - 메서드 : 클래스에 대한 객체의 행동을 구현
...
- 부모 클래스가 있는 경우 extends 키워드 다음에 부모 클래스의 이름을 붙입니다. 클래스는 단 하나의 상위 클래스만 상속할 수 있습니다. 자바는 다중상속을 지원하지 않음

- implements 키워드 뒤에 인터페이스들의 목록이 올 수 있습니다. 클래스는 여러 인터페이스를 동시에 구현할 수 있음

- 다중상속은 불가능하지만, 여러개의 인터페이스를 하나의 클래스에서 동시에 구현 가능함.
...
- 위에서 만든 chicken 객체를 생성하는 코드이며 이 한줄에 의해 담긴 이야기를 해보겠습니다.
	...
	- 여기서 new 연산자는 새로운 객체를 위한 메모리를 할당하고 && 그 메모리에 대한 참조를 반환하는 연산자입니다. 
	- new 연산자는 피연산자로 객체의 생성자를 사용해서 만드는데, 코드에 생성자가 없다면, 디폴트 컨스트럭터가 만들어져서 알아서 생성됨.
